...
`src/_locales/zh_TW/messages.json`. Edit it there and keep this copy identical. 22 characters.

```
StampStack — 廣告與追蹤器攔截器
```
...
• 開始購買或還原購買後，ExtensionPay 的程式庫會把授權金鑰（付款或登入後還包括購買電子郵件）儲存在 Chrome 的同步儲存空間中，Chrome 會將其同步到你已登入並開啟同步的其他瀏覽器
• 隨時可以把設定匯出為檔案，再重新匯入
