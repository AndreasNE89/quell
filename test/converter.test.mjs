...
test('entity-wildcard $to (requestDomains) with no valid entry is skipped', () => {
  ...
  assert.equal(dnr.rule, undefined);
  assert.equal(dnr.skip, 'invalid-request-domains');
});
...
test('uBO short-form $ghide/$ehide/$shide map to cosmetic exceptions', () => {
  assert.equal(convert('@@||example.com^$ghide').dnr.cosmeticException, 'generichide');
