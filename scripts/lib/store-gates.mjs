...
// build shipped the local slug while every gate reported the tracked id. A regex also misses
// every other form the runtime honours (a template literal, `let`, `export { … }`, `as const`).
...
/**
 * `{ tracked, override }` — each a usable id or null. `tracked` is what a store bundle carries;
 * `override` is the gitignored local value, which only dev builds honour.
 */
  ...
  const out = await build({
    stdin: {
      contents:
        ...
        // By absolute path, so the stub below (which matches the relative specifier) leaves this
        // import alone. A namespace read: a local file without the export reads as no override
        // instead of failing the build.
        (existsSync(local)
          ? `import * as local from ${JSON.stringify(local)};\n` +
            `export const EXTPAY_EXTENSION_ID_OVERRIDE = local.EXTPAY_EXTENSION_ID_OVERRIDE ?? null;\n`
    ...
    write: false,
    logLevel: 'silent',
    // extpay-config.ts's own import of the local file is stubbed as in a store bundle, so the
    // tracked id reads the same whatever the gitignored file holds, lacks, or whether it exists.
    plugins: [storeExtPayLocalStub()],
  });
 ...
 * fails closed rather than passing on nothing (test/store-gates.test.mjs builds the real worker
 * both ways, so such a change shows up in the tests before it blocks a release).
 */
